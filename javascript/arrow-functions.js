...
function getSaleProducts() {
  let newArr = products.map(e => {
    const discountedPrice = e.price * 0.75 // 25% discount
    return { ...e, price: discountedPrice }
  })
  return newArr
}
...
function getBlueProducts() {
  const blueProducts = products.filter(e => e.color.includes('blue'))
  return blueProducts
}

// Get the total price of all the products using the reduce method.
function getTotalPrice() {
  return products.reduce((acc, arr) => acc + arr.price, 0)
}
